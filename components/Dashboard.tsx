  ...
  return (
    <div className="p-4 md:p-10 max-w-7xl mx-auto pb-24 md:pb-10">
      <header className="flex flex-col gap-4 mb-6 md:mb-10">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight">견적서 관리</h2>
        ...
        <div className="flex items-center gap-2 md:gap-3 flex-wrap">
          {viewMode === "monthly" && (
            <div className="flex items-center gap-1.5 bg-white p-1.5 rounded-xl border border-neutral-200 shadow-sm flex-1 md:flex-none">
              <Calendar size={16} className="text-neutral-400 ml-2 flex-shrink-0" />
              <select
                value={selectedYear}
                onChange={(e) => setSelectedYear(Number(e.target.value))}
                className="bg-transparent border-none text-sm font-medium focus:ring-0 cursor-pointer min-w-0"
              >
                {years.map((y) => (
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 md:px-4 py-2 md:py-2.5 rounded-xl font-medium text-sm transition-colors shadow-sm whitespace-nowrap ${
              viewMode === "all"
          ...
          <button
            onClick={onCreateNew}
            className="hidden md:flex bg-neutral-900 text-white px-5 py-2.5 rounded-xl font-medium hover:bg-neutral-800 transition-colors items-center gap-2 shadow-sm whitespace-nowrap"
          >
      ...
      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4 mb-6 md:mb-10">
        <div className="bg-white p-4 md:p-5 rounded-2xl border border-neutral-200 shadow-sm col-span-2 lg:col-span-1">
        ...
        <div className="bg-white p-4 md:p-5 rounded-2xl border border-neutral-200 shadow-sm">
          <div className="text-neutral-500 text-xs font-medium mb-1">
            견적 건수
          </div>
          <div className="text-xl md:text-2xl font-bold tracking-tight">
        ...
        <div className="bg-white p-4 md:p-5 rounded-2xl border border-neutral-200 shadow-sm">
          ...
          <div className="text-xl md:text-2xl font-bold tracking-tight text-emerald-600">
      ...
      <div className="bg-white rounded-2xl border border-neutral-200 shadow-sm overflow-hidden">
        <div className="p-4 md:p-5 border-b border-neutral-200 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-neutral-50/50">
          <h3 className="font-semibold text-lg">
            {viewMode === "all" ? "전체 견적서" : "견적서 목록"}
            <span className="text-sm font-normal text-neutral-400 ml-2">{filteredQuotes.length}건</span>
          </h3>
          <div className="relative">
            ...
            <input
              ...
              className="w-full sm:w-64 pl-9 pr-4 py-2 bg-white border border-neutral-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-neutral-900/10 focus:border-neutral-900 transition-all"
            />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-neutral-500 flex flex-col items-center">
            ...
            <p className="text-lg font-medium text-neutral-900 mb-1">
              {viewMode === "all" ? "작성된 견적서가 없습니다" : "해당 월에 작성된 견적서가 없습니다"}
            </p>
          ...
          <>
            ...
            <div className="md:hidden divide-y divide-neutral-100">
              {filteredQuotes.map((quote) => (
                <div
                  key={quote.id}
                  className="p-4 active:bg-neutral-50 transition-colors cursor-pointer"
                  onClick={() => onView(quote)}
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-medium text-base">{quote.client.name}</span>
